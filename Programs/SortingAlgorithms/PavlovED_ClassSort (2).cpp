...
out << setw(10) << left << x.Job;//по левому краю, 10 позиций для фамилии
...
out << left << setw(10) << x.JobTime;//запрлата
...
void radixSortAlgo(vector<people>& x, int sizex){
    ...
    vector< vector<people> >a(11);
        for(int i=0;i<digitsCount;i++){  // Создание нового массива
        for(int j=0;j<sizex;j++){
            int s = (x[j].Salary/int(pow(10,i))%10);
            a[s].push_back(x[j]);
        }
        x.clear();
        for (int k=0;k<10;k++){
